...
//		this.overlapsSplitSpanAnnotations = new ArrayList<Annotation>();
	...
	protected List<InlineTag> getInlineTags(Annotation annotation) {
		...
		if (!type.equals("syntactic context")) {
			...
			if (annotationSpans.size() == 1) {
				...
				// }
				logger.warn(String
						.format("Although the GENIA inline term annotation representation allows for discontinous annotations, "
				...
				addExcludedAnnotation(annotation);
			}
		}
