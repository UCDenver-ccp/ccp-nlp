 ...
 * 
 */
public class ElsevierArt501AoDocumentRdfGenerator extends AoDocumentRdfGenerator {
	private static final Pattern pmcidPattern = Pattern.compile("-(\\d+)\\.nxml");

	private static final String BASE_URL = "http://elsevier.com/xxx/";
			...
			if (m.find(m.end(1)))
				throw new IllegalArgumentException(
						"Ambiguous document ID, unable to determine which string of numbers is the PubMed Central ID in: "
								+ documentId);
			try {
				documentUrl = new URL(BASE_URL + pmcid);
				URI documentUri = getUri(new PubMedCentralID(pmcid));
	 ...
	 * @return
	 */
	protected URI getUri(PubMedCentralID pmcid) {
